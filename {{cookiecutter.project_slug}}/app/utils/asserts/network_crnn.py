...
class CRNN(nn.Module):
    ...
    def __init__(self, imgH, nc, nclass, nh, leakyRelu=False,lstmFlag=True):
        """
        是否加入lstm特征层
        """
...
if __name__ == "__main__":
    ...
    ocrModel = "checkpoints/ocr-english.pth"
    alphabet = '01234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ*'
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using Device {device}")
    
    # 定义网络
    crnn = CRNN(32, 1, len(alphabet)+1, 256, lstmFlag=True).cuda()
    # 加载权重
    trainWeights = torch.load(ocrModel,map_location="cuda")
    modelWeights = OrderedDict()
    for k, v in trainWeights.items():
        name = k.replace('module.','') # remove `module.`
        modelWeights[name] = v
    crnn.load_state_dict(modelWeights)
    crnn.eval()
